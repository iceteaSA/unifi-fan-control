...
The sandboxed suite has 11 tests after Phase 3. CI tests Bash 4.4, 5.1, 5.2, and native Ubuntu under mawk. The ShellCheck baseline in `.github/shellcheck-baseline.tsv` is temporary debt until Phase 2; it does not permit adding new warnings.
...
- `ci/description` - CI and release automation changes

### Releases

Releases are tagged `vX.Y.Z` and generated by Release Please from conventional
commits merged to `main`. The release workflow verifies the tagged commit,
publishes the four runtime files as a tarball, and uploads `SHA256SUMS`.
`VERSION` contains the bare SemVer value, and the daemon logs
`CONFIG: fan-control vX.Y.Z starting` at startup. Installer version pinning is
planned for a later phase and is not part of the current installer.
