...
The sandboxed suite has 15 tests. CI tests Bash 4.4, 5.1, 5.2, and native Ubuntu under mawk. ShellCheck and shfmt must report zero findings.
...
`CONFIG: fan-control vX.Y.Z starting` at startup. Set `FAN_CONTROL_VERSION`
to install a checksum-verified release; `FAN_CONTROL_BRANCH` installs an
unverified branch.
